...
func RecomputeProduction(ctx context.Context, tx Tx, settlementID uuid.UUID) error {
	...
	for good := range weakestLinkRefiningBuilding {
		...
		for _, opt := range buildingOptions {
			...
			refiningCapacity += placementYield(good, rate, opt.CapL1PerGood[good], opt.PlaceCapPerGood[good], opt.MultPerGood[good], placed)
		}
	...
	for _, opt := range buildingOptions {
		...
		bronzeRefiningCapacity += placementYield(GoodBronze, rate, opt.CapL1PerGood[GoodBronze], opt.PlaceCapPerGood[GoodBronze], opt.MultPerGood[GoodBronze], placed)
	}
