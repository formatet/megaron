...
func TestKharisNeverSpent(t *testing.T) {
	for id, spec := range PrayerSpecs {
		...
		// Verify it maps to one of the three canonical tier constants —
		// built from the constants themselves, not restated as literals,
		// so this test cannot pass by mirroring the same drift it checks for.
		validThresholds := map[float64]bool{MoodSuspicious: true, MoodIndifferent: true, MoodFavorable: true}
		if !validThresholds[spec.MinKharis] {
			t.Errorf("prayer %q: MinKharis = %.0f is not a recognised tier (%.0f/%.0f/%.0f)",
				id, spec.MinKharis, MoodSuspicious, MoodIndifferent, MoodFavorable)
		}
	}
}
