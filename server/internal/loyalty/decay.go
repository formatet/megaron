...
func checkRevolt(ctx context.Context, pool *pgxpool.Pool, settlementID uuid.UUID) {
	...
	var infantry, cavalry, catapult, priest, ship int
	err := pool.QueryRow(ctx,
		`SELECT loyalty, owner_id, infantry, cavalry, catapult, priest, ship
		 FROM settlements WHERE id = $1`,
		settlementID,
	).Scan(&loyalty, &ownerID, &infantry, &cavalry, &catapult, &priest, &ship)
	if err != nil || loyalty > 1 {
		return
	}

	total := infantry + cavalry + catapult + priest + ship
	ownerFraction := 1.0
	if total > 0 {
