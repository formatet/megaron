...
Progressive disclosure:
  poleia actions              category overview with available/locked counts
  poleia actions <category>   drill into one category's verbs, grouped
                               Available now / Locked — with requirement,
                               live gap, and how to unlock it
  poleia actions --json       the full raw array (all verbs, all categories)

Categories: province, military, trade, diplomacy, kingdom, cult`,
		Example: `  poleia actions
  poleia actions military
  poleia actions military --province <province-id>
  poleia actions --json`,
...
func printCategoryOverview(verbs []actionVerb) {
	...
	fmt.Println("What you can do — by category (poleia actions <category> for details)")
	fmt.Println(strings.Repeat("─", 60))
