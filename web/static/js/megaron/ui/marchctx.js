...
export async function openMarchCtx(dest, screenX, screenY) {
  ...
  // Eligible: garrisoned/positioned units start a plain march. A unit already
  // MARCHING is eligible too — right-clicking a new destination sends it a
  // redirect via Runner instead (Timothy 2026-09-25: "it doesn't seem
  // possible to give orders to units that have already been given orders —
  // that is wrong, they must be reachable by orders"). The server already
  // accepts this (POST .../recall with a target — combat/march_start.go
  // names it explicitly); the map menu was the only thing hiding it.
  ...
  // (march_start.go:132-135) and must not show as eligible here either — a
  // marching unit can never be in fortify stance, so that check only applies
  // to the garrison/positioned branch.
  const wantNaval = dest.isSea;
  State.marchCtxUnits = all.filter(u => (u.category === 'naval') === wantNaval && marchCtxOrderMode(u) !== null);
