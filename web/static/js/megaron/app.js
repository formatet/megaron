// fmtSilver — copied verbatim from web/templates/base.html (defined there in the
// inline <head> <script> for index/join/epitaph; duplicated here because this
// file no longer shares a document with base.html after the FAS 1 frikoppling).
function fmtSilver(v) {
  v = Math.round(v);
  if (v === 0) return '0 shekel';
  if (v < 60)  return v + ' shekel';
  if (v < 3600) {
    const mina = Math.floor(v / 60), shekel = v % 60;
    return shekel ? mina + ' mina ' + shekel + ' shekel' : mina + ' mina';
  }
  const talang = Math.floor(v / 3600), rest = v % 3600;
  const mina   = Math.floor(rest / 60);
  return mina ? talang + ' talang ' + mina + ' mina' : talang + ' talang';
}

const WORLD_ID = window.WORLD_ID;
const MY_SETTLEMENT_ID = window.MY_SETTLEMENT_ID;
const MY_PLAYER_ID = window.MY_PLAYER_ID;
const WORLD_CREATED_AT = window.WORLD_CREATED_AT;
const TIME_SCALE = window.TIME_SCALE;
...
let messengerData = [];
let tradeData = [];
let unitsData = [];  // per-unit armies/fleets (units table) — drawn on the canvas
...
function ownCapital() {
  return provinceData.find(p => p.own && p.is_capital)
      || provinceData.find(p => p.own && !p.is_outpost)
      || provinceData.find(p => p.own)
      || null;
}
...
// prev/next arrows (cycleCityView) or by clicking a settlement on the map.
let cityViewID = null; // province ID of the settlement the City drawer currently shows
function activeCitySettlement() {
  const mine = provinceData.filter(p => p.own && !p.is_outpost);
  if (!mine.length) return null;
  const chosen = cityViewID && mine.find(p => p.id === cityViewID);
  return chosen || mine.find(p => p.is_capital) || mine[0];
}
function cycleCityView(dir) {
  const mine = provinceData.filter(p => p.own && !p.is_outpost);
  if (mine.length < 2) return;
  const current = activeCitySettlement();
  let idx = mine.findIndex(p => p.id === current.id);
  idx = (idx + dir + mine.length) % mine.length;
  cityViewID = mine[idx].id;
  loadDrawerContent('city');
}
...
function render() {
  ...
  if (!dirty && !seaChanged && marchData.length === 0 && messengerData.length === 0 && tradeData.length === 0
      && !unitsData.some(u => u.status === 'marching')) {
...
async function loadMap() {
  ...
  if (unitsRes.ok) {
    unitsData = (await unitsRes.json()).units || [];
  }
  if (msgRes.ok) {
...
// Refetch the fog-of-war map (tileData) WITHOUT recentring the camera. Fog
...
function refreshTiles() {
  fetchAuth(`/api/v1/worlds/${WORLD_ID}/map`).then(r => r.ok && r.json().then(d => { tileData = d; dirty = true; }));
}
...
setInterval(() => {
  ...
  fetchAuth(`/api/v1/worlds/${WORLD_ID}/units`).then(r => r.ok && r.json().then(d => { unitsData = d.units || []; dirty = true; }));
}, 30000);
...
setInterval(() => {
  if (!unitsData.some(u => u.status === 'marching')) return;
  refreshTiles();
  fetchAuth(`/api/v1/worlds/${WORLD_ID}/units`).then(r => r.ok && r.json().then(d => { unitsData = d.units || []; dirty = true; }));
}, 3000);
  ...
  function connect() {
    ...
    ws.onmessage = e => {
      ...
      if (msg.kind === 'ColonyFounded') {
        ...
        addNotifChip('city', '🏛', notifText('ColonyFounded', p) + (grainLine ? ' — ' + grainLine : ''), 'now');
      }
      ...
      if (msg.kind === 'KharisEvent') {
        addNotifChip('kult', '⛩', msg.payload?.message || 'Divine event', 'now');
      }
      if (msg.kind === 'UnitAttrition' || msg.kind === 'UnitDeserted') {
        // Units bleeding out from grain/silver shortage — previously silent.
        addNotifChip('war', notifIcon(msg.kind), notifText(msg.kind, msg.payload || {}), 'now');
        fetchAuth(`/api/v1/worlds/${WORLD_ID}/units`).then(r => r.ok && r.json().then(d => { unitsData = d.units || []; dirty = true; }));
      }
...
let marchCtxDest   = null;  // { q, r, terrain, isSea, name, isSettlement, allied }
let marchCtxUnits  = [];    // eligible units currently listed
let marchCtxGroups = [];    // eligible units grouped by type+origin (quantity picker)
...
async function onColonizeToggle() {
  const chk = document.getElementById('mctx-colonize-chk');
  const nameEl = document.getElementById('mctx-colony-name');
  ...
  const prevEl = document.getElementById('mctx-colonize-preview');
  if (!prevEl) return;
  if (!(chk && chk.checked) || !marchCtxDest) {
    prevEl.style.display = 'none';
    prevEl.innerHTML = '';
    return;
  }
  const dest = marchCtxDest;
  prevEl.style.display = 'block';
  prevEl.innerHTML = '<span style="color:var(--text-dim)">Reading the land…</span>';
  try {
    const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/colonize-preview?q=${dest.q}&r=${dest.r}`);
    if (!r.ok) throw new Error();
    const p = await r.json();
    if (marchCtxDest !== dest) return; // the menu moved on while we fetched
    prevEl.innerHTML = renderColonizePreviewHTML(p);
  } catch (_) {
    if (marchCtxDest === dest) prevEl.innerHTML = '<span style="color:var(--text-dim)">No forecast available.</span>';
  }
}
...
async function openMarchCtx(dest, screenX, screenY) {
  marchCtxDest  = dest;
  marchCtxUnits = [];
  document.getElementById('mctx-err').textContent = '';
  ...
  const res = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/units`);
...
function renderMarchUnitList() {
  ...
  marchCtxGroups = Array.from(byKey.values());
  el.innerHTML = marchCtxGroups.map((g, i) => {
  ...
  stanceRow.style.display = (marchCtxDest && marchCtxDest.isSea) ? 'none' : 'block';
}

async function sendMarch() {
  if (!marchCtxDest) return;
  const picks = [];
  marchCtxGroups.forEach((g, i) => {
  ...
  const stance = marchCtxDest.isSea ? '' : (document.getElementById('mctx-stance').value || '');
  const chk = document.getElementById('mctx-colonize-chk');
  ...
  const results = await Promise.all(picks.map(uid => {
    const body = { target_q: marchCtxDest.q, target_r: marchCtxDest.r };
    ...
    if (marchCtxDest.isSea) body.intent = 'explore';
    return fetchAuth(`/api/v1/worlds/${WORLD_ID}/units/${uid}/march`, {
...
canvas.addEventListener('contextmenu', e => {
  ...
  const tile = tileData.find(t => t.q === h.q && t.r === h.r);
  if (!tile || tile.terrain === 'fog') { closeMarchCtx(); return; }
...
async function loadDrawerContent(name) {
  ...
  if (name === 'city') {
    ...
    const mineCount = provinceData.filter(p => p.own && !p.is_outpost).length;
...
function notifShowKind(kind) { loadNotifDrawer(kind || null); }

async function loadNotifDrawer(kindFilter) {
  const body = document.getElementById('notif-body');
  body.innerHTML = '<div class="loading" style="padding:.5rem">Loading…</div>';
  try {
    const base = `/api/v1/worlds/${WORLD_ID}/notifications`;
...
// ── Economy drawer ────────────────────────────────────────────────────────
async function loadEconomyDrawer() {
...
async function loadTransferGoods(fromProvId) {
  const sel = document.getElementById('ec-tr-good');
  if (!sel || !fromProvId) return;
  sel.innerHTML = '<option value="">Loading…</option>';
  const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/provinces/${fromProvId}/goods`);
...
async function startTransfer() {
  ...
  const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/provinces/${from}/trade`, {
...
async function loadEconomyWants() {
  ...
  el.innerHTML = '<div class="loading" style="padding:.5rem">Loading…</div>';
  try {
    const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/market/wants`);
...
async function loadKultDrawer() {
  ...
  try {
    const [sr, pr] = await Promise.all([
      fetchAuth('/api/v1/worlds/' + WORLD_ID + '/settlements/' + MY_SETTLEMENT_ID),
      fetchAuth('/api/v1/worlds/' + WORLD_ID + '/provinces/' + capital.id),
    ]);
...
async function okRite(prayerID) {
  const r = await fetchAuth('/api/v1/worlds/' + WORLD_ID + '/settlements/' + MY_SETTLEMENT_ID + '/rite', {
...
async function startCraft(provinceID) {
  ...
  resultEl.textContent = '';
  const r = await fetchAuth(
    `/api/v1/worlds/${WORLD_ID}/provinces/${provinceID}/craft`,
...
async function loadTicklog() {
  ...
  try {
    const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/provinces/${capital.id}/ticklog?last=10`);
...
async function cancelBuild(provinceID, queueID) {
  const res = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/provinces/${provinceID}/build-queue/${queueID}`, { method: 'DELETE' });
  if (res.ok) {
    await refreshCityBuildings(provinceID);
  } else {
...
async function warRecruitShip(unitType) {
  const sel = document.getElementById('war-recruit-city');
  const pid = sel ? sel.value : null;
  if (!pid) return;
  ...
  const res = await fetchAuth('/api/v1/worlds/' + WORLD_ID + '/provinces/' + pid + '/recruit', {
    method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body),
  });
  const data = await res.json().catch(() => ({}));
  const resEl = document.getElementById('war-recruit-res');
...
async function warAbandon(settlementID, name) {
  if (!confirm('Abandon ' + name + '? This cannot be undone.')) return;
  const res = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/settlements/${settlementID}/abandon`, { method: 'POST' });
...
async function unitRecall(unitID) {
  const statusEl = document.getElementById('uorder-' + unitID);
  const res = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/units/${unitID}/recall`, {
...
function unitRedirectToggle(unitID) {
  const row = document.getElementById('uredir-' + unitID);
  if (row) row.style.display = row.style.display === 'none' ? 'flex' : 'none';
}

async function unitRedirect(unitID) {
...
async function loadDipCities() {
  ...
  el.innerHTML = '<div class="loading" style="font-size:.8rem">Loading…</div>';
  try {
    const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/cities`);
    if (!r.ok) throw new Error();
...
async function loadDipRulers() {
  ...
  el.innerHTML = '<div class="loading" style="font-size:.8rem">Loading…</div>';
  try {
    const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/diplomacy`);
    if (!r.ok) throw new Error();
...
async function dipCancel(id, btn) {
  btn.disabled = true;
  const res = await fetchAuth('/api/v1/worlds/' + WORLD_ID + '/messengers/' + id + '/trade-cancel', {
    method: 'POST', headers: {'Content-Type':'application/json'}, body: '{}',
  });
  if (res.ok) {
    await loadDipThreads();
...
async function renderLockedActions(category, provinceID) {
  const id = provinceID || (ownCapital() || {}).id;
  if (!id) return '';
  try {
    const r = await fetchAuth(`/api/v1/worlds/${WORLD_ID}/provinces/${id}/actions`);
    if (!r.ok) return '';
...
function colonyFoundedGrainLine(body) {
